...
def approx_average_is_average(hand:list[int]) -> bool:
    ...
    """
    strategy1: float = (hand[0] + hand[-1]) / 2
    strategy2: int = hand[len(hand) // 2]
    return card_average(hand) == strategy1 or card_average(hand) == strategy2
...
def average_even_is_average_odd(hand:list[int]) -> bool:
    ...
    even_avg: float = card_average(hand[0::2])
    odd_avg: float = card_average(hand[1::2])
